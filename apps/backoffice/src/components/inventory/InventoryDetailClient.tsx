...
export function InventoryDetailClient(props: {
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between">
        <div>
          ...
          <div className="mt-1 flex flex-wrap items-center gap-2">
            <h1 className="font-display text-xl font-semibold tracking-tight">{props.item.public_no}</h1>
            <span
            ...
            <span className="text-xs text-muted-foreground">{presentInventoryChannel(props.item.product_channel)}</span>
          </div>
          ...
          {badges.length > 0 ? (
            <div className="mt-2 flex flex-wrap gap-1">
              {badges.map((b) => (
                <span
                  key={b}
                  className="rounded-full bg-status-warn px-2 py-0.5 text-xs font-medium text-status-warn-foreground ring-1 ring-status-warn"
                >
                  {b}
                </span>
              ))}
      ...
      {msg ? <div className="rounded-xl border border-status-danger bg-status-danger px-3 py-2 text-sm text-status-danger-foreground">{msg}</div> : null}

      <section className="rounded-2xl border border-border bg-surface p-3 md:p-4">
        <h2 className="mb-2 font-display text-sm font-semibold">合规 / IMEI</h2>
        ...
        {props.item.product_channel === "trade_in" && holdActive && props.item.listing_hold_until ? (
          <p className="mt-2 text-xs text-status-warn-foreground">
            冷冻期至{" "}
      ...
      <section className="rounded-2xl border border-border bg-surface p-3 md:p-4">
        <h2 className="mb-3 font-display text-sm font-semibold">质检（分项）</h2>
        <div className="space-y-4">
          <QaBlock
            disabled={busy}
        ...
        {props.item.qa_completed_at ? (
          <p className="mt-2 text-xs text-status-success-foreground">
            质检已完成 ·{" "}
          ...
          <p className="mt-2 text-xs text-muted-foreground">回收机须标记质检完成后才可售。</p>
        )}
      </section>
      ...
      <section className="rounded-2xl border border-border bg-surface p-3 md:p-4">
        <h2 className="mb-2 font-display text-sm font-semibold">关联客户</h2>
        <div className="grid grid-cols-1 gap-2 text-sm md:grid-cols-2">
          <div className="rounded-xl border border-border bg-surface-2 p-3">
            <div className="text-xs font-medium text-muted-foreground">卖方 / 回收来源</div>
            <div className="mt-1 text-foreground">{props.item.sellerLabel ?? "—"}</div>
          </div>
          <div className="rounded-xl border border-border bg-surface-2 p-3">
            <div className="text-xs font-medium text-muted-foreground">买方（售出后）</div>
            <div className="mt-1 text-foreground">{props.item.buyerLabel ?? "—"}</div>
          </div>
        </div>
      </section>

      <section className="rounded-2xl border border-border bg-surface p-3 md:p-4">
        <h2 className="mb-3 font-display text-sm font-semibold">操作</h2>
      ...
      <section className="rounded-2xl border border-border bg-surface p-3 md:p-4">
        <h2 className="mb-3 font-display text-sm font-semibold">操作时间线</h2>
        ...
        <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/35 p-0 md:items-center md:p-4">
          <div className="flex max-h-[85dvh] w-full flex-col rounded-t-2xl border border-border bg-surface shadow-lg md:max-h-[85vh] md:max-w-lg md:rounded-2xl">
            <div className="flex items-center justify-between border-b border-border px-4 py-3">
              ...
              <button className="rounded-xl px-2 py-1 text-sm text-muted-foreground" onClick={() => setOpen(false)} type="button">
                关闭
              </button>
            </div>
            <div className="flex-1 space-y-3 overflow-y-auto px-4 py-4">
              <div>
                <label className="mb-0.5 block text-[11px] text-muted-foreground">买方电话</label>
                <input
              ...
              <div>
                <label className="mb-0.5 block text-[11px] text-muted-foreground">买方姓名（可选）</label>
                <input
                  className="ui-input h-10 w-full md:h-9"
              ...
              <div>
                <label className="mb-0.5 block text-[11px] text-muted-foreground">成交价 EUR（可选）</label>
                <input
                  className="ui-input h-10 w-full md:h-9"
                  inputMode="decimal"
