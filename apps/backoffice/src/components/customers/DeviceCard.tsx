...
export function DeviceCard({ customerId, devices }: Props) {
  ...
  return (
    <section className="rounded-2xl border border-border bg-surface p-4">
      <h2 className="mb-3 text-sm font-semibold text-neutral-900">设备 ({devices.length})</h2>
      {devices.length === 0 ? (
  ...
  return (
    <div className="rounded-xl border border-border bg-surface-2 p-3">
      <div className="flex items-start justify-between">
        <div>
          <div className="text-sm font-medium text-neutral-900">{device.brand} {device.model}</div>
          {device.serialOrImei && (
            <div className="mt-0.5 text-xs text-neutral-500">IMEI/SN: {device.serialOrImei}</div>
          )}
        </div>
        <div className="flex gap-2">
          <button className="text-[11px] text-indigo-600 hover:underline" onClick={onEdit} type="button">编辑</button>
          <button className="text-[11px] text-rose-500 hover:underline disabled:opacity-60" disabled={deleting} onClick={handleDelete} type="button">
            {deleting ? "..." : "删除"}
          </button>
        </div>
      </div>
