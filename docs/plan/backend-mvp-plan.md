...
- 已完成：首页工作台结构对齐（统计条、待办卡、快捷入口、最近活动）并支持跳转到工单筛选页
- 已完成：工单列表 query 筛选接入（状态/类型/技师/结清/日期/待确认超时/超期未取件）
- 已完成：状态展示抽离为共享模块（统一文案与颜色规则）
- 已完成：新增最小状态流转接口 `POST /api/orders/:id/transition`
- 已完成：状态流转后写入 `order_events` 审计记录
- 已验证：`npm run lint` 与 `npm run build` 通过
